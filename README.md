...
| `channel` | tidak | Filter channel penjualan: `shopify` / `shopee` / `tokopedia` / `tiktok` / `internal`. Toleran typo (`shopfy`, `tokpedia`) + sinonim (`tokped`, `toped`, `tik tok`, `tt`, `web`, `web resmi`, `website`). Channel `internal` mencakup order WhatsApp/chat: `WS` = Wholesale (`wholesale`, `grosir`) dan `DP` = Direct Phone (`telepon`, `dp`) — sinonim `wa`, `whatsapp`, `chat` juga mengarah ke sana. Difilter SEBELUM scoring, jadi juga memecah ambiguitas antar-channel ("Komang di Shopee" vs "Komang di Tokopedia"). **Channel relaxation:** kalau filter membuat hasil nol padahal nama ditemukan yakin di channel lain (pelanggan sering salah sebut — TikTok Shop tampil sebagai "Shop \| Tokopedia"), hasil tetap dikembalikan dengan `channel_matched: false` + `channel_note`. Channel tak dikenal → `400` + hint. Alias: `kanal` / `marketplace` |
